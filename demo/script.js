...
$.fn.switchBackgroundColor = function() {
  console.log('old color was ' + currentColor);
  ...
  console.log('updated color is ' + currentColor);
}

$.fn.switchButtonsDataColorValue = function() {
  const $_THIS = $(this);
	if( currentColor === 'pink' ) {
  	$_THIS.attr('data-color', 'pink');
  } else {
  	$_THIS.attr('data-color', 'green');  
  }
}

$('body').css('background', currentColor);

const btnArray = [$('#btn01'), $('#btn02'), $('#btn03')];

$(document).similarElements({
	arrayOfSimilarElements: btnArray,
  triggerEvent: 'click',
  triggeredElement: function() {
  	console.log('triggeredElement: ' + $(this).attr('id'));
    $(this).switchBackgroundColor();
    $(this).switchButtonsDataColorValue();
  },
  nonTriggeredElement: function() {
  	console.log('nonTriggeredElement: ' + $(this).attr('id'));
   $(this).switchButtonsDataColorValue();
  }
});
